...
>>>>>>> Stashed changes
    ...
    upload_raw = sensel_raw.get("upload") or {}
    events_ingest_path = os.environ.get("SENSEL_EVENTS_INGEST_PATH", "").strip()
    if events_ingest_path:
        upload_raw["events_path"] = events_ingest_path
    sensel_raw["upload"] = upload_raw
...
=======
    event_env_defaults = {
        "watch_path": ("SECURITY_EVENTS_PATH", "/app/data/assets/security-events.jsonl"),
        "offset_path": ("SECURITY_EVENTS_OFFSET", "/app/data/security-events.offset"),
        "snort_watch_path": ("SNORT_EVENTS_PATH", "/app/data/assets/snort-events.jsonl"),
        "snort_offset_path": ("SNORT_EVENTS_OFFSET", "/app/data/snort-events.offset"),
        "suricata_watch_path": ("SURICATA_EVENTS_PATH", "/app/data/assets/suricata-events.jsonl"),
        "suricata_offset_path": ("SURICATA_EVENTS_OFFSET", "/app/data/suricata-events.offset"),
    }
    for key, (env_name, default_value) in event_env_defaults.items():
        env_value = os.environ.get(env_name, "").strip()
        if env_value:
            events_raw[key] = env_value
        else:
            events_raw.setdefault(key, default_value)
    sensel_raw["events"] = events_raw
    sensel_raw["health_interval_sec"] = int(
        capture_raw.get("health_check_interval_sec", 30)
    )
