"""Unit tests for the Suricata EVE JSON -> SecurityEvent bridge."""

from __future__ import annotations

import json

from src.detection.external_engine.suricata_source import (
    SuricataEveMapper,
    SuricataEveSource,
    map_severity,
    parse_eve_timestamp,
)

_MODBUS_ALERT = {
    "timestamp": "2026-06-18T10:30:00.123456+0000",
    "flow_id": 1234567890,
    "event_type": "alert",
    "src_ip": "10.10.1.20",
    "src_port": 55321,
    "dest_ip": "10.10.1.100",
    "dest_port": 502,
    "proto": "TCP",
    "app_proto": "modbus",
    "alert": {
        "action": "allowed",
        "gid": 1,
        "signature_id": 100001,
        "rev": 1,
        "signature": "Unauthorized Modbus TCP access",
        "category": "Attempted Administrator Privilege Gain",
        "severity": 1,
    },
}

_FLOW_RECORD = {
    "timestamp": "2026-06-18T10:30:01.000000+0000",
    "flow_id": 222,
    "event_type": "flow",
    "src_ip": "10.10.1.20",
    "dest_ip": "10.10.1.100",
    "proto": "TCP",
}


def test_map_alert_fields():
    mapper = SuricataEveMapper(site_id="factory-a", sensor_id="ndr-edge-001")
    event = mapper.map(_MODBUS_ALERT)
    payload = event.to_dict()

    assert payload["rule_id"] == "suricata-1-100001"
    assert payload["severity"] == "high"
    assert payload["risk_score"] == 85
    assert payload["protocol"] == "tcp"
    assert payload["src_ip"] == "10.10.1.20"
    assert payload["dst_ip"] == "10.10.1.100"
    assert payload["dst_port"] == 502
    assert payload["event_type"] == "SURICATA_ALERT"
    assert payload["description"] == "Unauthorized Modbus TCP access"
    assert payload["evidence"]["engine"] == "suricata"
    assert payload["evidence"]["sid"] == 100001
    assert payload["evidence"]["app_proto"] == "modbus"
    assert payload["evidence"]["raw_event"] == _MODBUS_ALERT
    assert payload["event_id"].startswith("evt-")
    assert "-suricata-" in payload["event_id"]


def test_non_alert_record_is_skipped():
    mapper = SuricataEveMapper(site_id="s", sensor_id="n")
    assert mapper.map(_FLOW_RECORD) is None


def test_event_id_increments():
    mapper = SuricataEveMapper(site_id="s", sensor_id="n")
    first = mapper.map(_MODBUS_ALERT).event_id
    second = mapper.map(_MODBUS_ALERT).event_id
    assert first != second


def test_map_severity_mapping():
    assert map_severity(1) == "high"
    assert map_severity(2) == "medium"
    assert map_severity(3) == "low"
    assert map_severity(None) == "medium"
    assert map_severity("bogus") == "medium"
    assert map_severity(9) == "medium"


def test_parse_eve_timestamp_normalizes_offset():
    ts = parse_eve_timestamp("2026-06-18T10:30:00.123456+0000")
    assert ts == "2026-06-18T10:30:00+00:00"


def test_parse_eve_timestamp_passthrough_colon_offset():
    iso = "2026-06-18T10:30:00+08:00"
    assert parse_eve_timestamp(iso) == iso


def test_parse_eve_timestamp_fallback_on_garbage():
    ts = parse_eve_timestamp("not-a-timestamp")
    assert ts.endswith("+00:00")


def test_missing_optional_fields_defaults():
    mapper = SuricataEveMapper(site_id="s", sensor_id="n")
    event = mapper.map({"event_type": "alert", "alert": {"signature_id": 5, "gid": 1}})
    payload = event.to_dict()
    assert payload["protocol"] == "ip"
    assert payload["severity"] == "medium"
    assert payload["description"] == "Suricata alert"
    assert "src_ip" not in payload  # empty src omitted by to_dict
    assert "dst_port" not in payload  # None omitted by to_dict


def _write_lines(path, records, *, trailing_newline=True):
    text = "\n".join(json.dumps(r) for r in records)
    if trailing_newline:
        text += "\n"
    path.write_text(text, encoding="utf-8")


def test_source_poll_writes_only_alert_events(tmp_path):
    src = tmp_path / "eve.json"
    out_dir = tmp_path / "assets"
    out_dir.mkdir()
    offset = tmp_path / "offset"
    # two alerts interleaved with a flow record that must be skipped
    _write_lines(src, [_MODBUS_ALERT, _FLOW_RECORD, _MODBUS_ALERT])

    source = SuricataEveSource(
        eve_json_path=str(src),
        output_dir=str(out_dir),
        offset_path=str(offset),
        site_id="s",
        sensor_id="n",
    )
    written = source.poll_once()
    assert written == 2

    lines = source.output_path.read_text(encoding="utf-8").splitlines()
    assert len(lines) == 2
    assert json.loads(lines[0])["rule_id"] == "suricata-1-100001"


def test_source_offset_only_reads_new_lines(tmp_path):
    src = tmp_path / "eve.json"
    out_dir = tmp_path / "assets"
    out_dir.mkdir()
    offset = tmp_path / "offset"
    _write_lines(src, [_MODBUS_ALERT])

    source = SuricataEveSource(
        eve_json_path=str(src),
        output_dir=str(out_dir),
        offset_path=str(offset),
        site_id="s",
        sensor_id="n",
    )
    assert source.poll_once() == 1
    assert source.poll_once() == 0  # nothing new

    with src.open("a", encoding="utf-8") as handle:
        handle.write(json.dumps(_MODBUS_ALERT) + "\n")
    assert source.poll_once() == 1


def test_source_preserves_partial_trailing_line(tmp_path):
    src = tmp_path / "eve.json"
    out_dir = tmp_path / "assets"
    out_dir.mkdir()
    offset = tmp_path / "offset"
    _write_lines(src, [_MODBUS_ALERT], trailing_newline=True)
    with src.open("a", encoding="utf-8") as handle:
        handle.write('{"event_type": "alert", "alert": {"signature_id": 7')  # partial
    source = SuricataEveSource(
        eve_json_path=str(src),
        output_dir=str(out_dir),
        offset_path=str(offset),
        site_id="s",
        sensor_id="n",
    )
    assert source.poll_once() == 1  # only the complete line

    with src.open("a", encoding="utf-8") as handle:
        handle.write(', "gid": 1}}\n')
    assert source.poll_once() == 1


def test_source_skips_malformed_lines(tmp_path):
    src = tmp_path / "eve.json"
    out_dir = tmp_path / "assets"
    out_dir.mkdir()
    offset = tmp_path / "offset"
    src.write_text("not json\n" + json.dumps(_MODBUS_ALERT) + "\n", encoding="utf-8")
    source = SuricataEveSource(
        eve_json_path=str(src),
        output_dir=str(out_dir),
        offset_path=str(offset),
        site_id="s",
        sensor_id="n",
    )
    assert source.poll_once() == 1


def test_source_handles_file_rotation(tmp_path):
    src = tmp_path / "eve.json"
    out_dir = tmp_path / "assets"
    out_dir.mkdir()
    offset = tmp_path / "offset"
    _write_lines(src, [_MODBUS_ALERT, _MODBUS_ALERT, _MODBUS_ALERT])
    source = SuricataEveSource(
        eve_json_path=str(src),
        output_dir=str(out_dir),
        offset_path=str(offset),
        site_id="s",
        sensor_id="n",
    )
    assert source.poll_once() == 3

    # Suricata rotated eve.json: now smaller than the saved offset.
    _write_lines(src, [_MODBUS_ALERT])
    assert source.poll_once() == 1


def test_source_missing_file_is_noop(tmp_path):
    source = SuricataEveSource(
        eve_json_path=str(tmp_path / "does-not-exist.json"),
        output_dir=str(tmp_path),
        offset_path=str(tmp_path / "offset"),
        site_id="s",
        sensor_id="n",
    )
    assert source.poll_once() == 0
