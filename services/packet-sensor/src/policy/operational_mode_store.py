"""Hot-reload operational mode artifact written by edge-agent."""

from __future__ import annotations

import json
import time
from pathlib import Path
from typing import Any

VALID_MODES = frozenset({"listen", "learning", "detect", "idle"})


class OperationalModeStore:
    def __init__(
        self,
        *,
        mode_path: str | Path,
        stamp_path: str | Path,
        reload_check_sec: float = 5.0,
        default_mode: str = "listen",
    ) -> None:
        self._mode_path = Path(mode_path)
        self._stamp_path = Path(stamp_path)
        self.reload_check_sec = reload_check_sec
        self._default_mode = default_mode if default_mode in VALID_MODES else "listen"
        self._last_check = 0.0
        self._last_stamp = ""
        self._artifact: dict[str, Any] = {}
        self.maybe_reload(force=True)

    @property
    def mode(self) -> str:
        self.maybe_reload()
        raw = str(self._artifact.get("mode") or self._default_mode).strip().lower()
        return raw if raw in VALID_MODES else self._default_mode

    @property
    def artifact(self) -> dict[str, Any]:
        self.maybe_reload()
        return dict(self._artifact)

    def maybe_reload(self, *, force: bool = False) -> bool:
        now = time.monotonic()
        if not force and (now - self._last_check) < self.reload_check_sec:
            return False
        self._last_check = now

        stamp = ""
        if self._stamp_path.is_file():
            try:
                stamp = self._stamp_path.read_text(encoding="utf-8").strip()
            except OSError:
                stamp = ""
        if not force and stamp == self._last_stamp and self._artifact:
            return False

        artifact: dict[str, Any] = {}
        if self._mode_path.is_file():
            try:
                raw = json.loads(self._mode_path.read_text(encoding="utf-8"))
                if isinstance(raw, dict):
                    artifact = raw
            except (json.JSONDecodeError, OSError):
                artifact = {}

        changed = stamp != self._last_stamp or artifact != self._artifact
        self._last_stamp = stamp
        self._artifact = artifact
        return changed

    def alerts_enabled(self) -> bool:
        return self.mode == "detect"

    def baseline_accumulation_enabled(self) -> bool:
        return self.mode == "learning"
